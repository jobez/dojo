...
async fn test_contract_deploy_via_udc() {
    ...
    let signer = LocalWallet::from(SigningKey::from_secret_scalar(
        FieldElement::from_hex_be(
            "0x01c30ee733a20acac5212c4eb47822da9ddf9a0eb284db5db9349bfb13fa4414",
        )
        .unwrap(),
    ));
    let address = FieldElement::from_hex_be(
        "0x00556c9b0839e725e04764087bb549cea66bf869d79d15a8eab10fbf2479270d ",
    )
        .unwrap();
    ...
    println!("class hash: {:#02X}", class_hash);
    
    let contract_factory = ContractFactory::new(class_hash, account);
    contract_factory
        .deploy(&vec![felt!("1"), felt!("2")], felt!("1"), false)
        .send()
        .await
